...
async function renderWeekView() {
    ...
    for (let d = 0; d < 7; d++) {
        try {
            ...
            console.error('Day render error:', err);
        }
    }
}
...
async function saveShift() {
    ...
    if (isDayOff) {
        ...
        const copyButtons = document.querySelectorAll('#copyDaysButtons .copy-day-btn.selected');
...
async function openDetailModal(shift) {
    ...
    if (photos.length > 0) {
        ...
        photos.forEach(photo => {
            ...
            html += '<img src="' + photo.data + '" onclick="openFullscreen(this.src)" alt="photo">';
            html += '<button class="detail-photo-delete" onclick="deletePhoto(' + photo.id + ')">X</button>';
            html += '</div>';
        });
        html += '</div>';
    } else {
